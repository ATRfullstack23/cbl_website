...
function InvoiceBarcodePrintHelper(config){
    var self = this;
    self.config = config || {};
    self.initialize();
    return self;
...
InvoiceBarcodePrintHelper.prototype = {
    constants: {
    },
    selectors: {
        container: "#invoiceBarcodePrintHelper",

        lblInvoiceNumber: "#lblInvoiceNumber",
    ...
    html:{
        container: '<dialog id="invoiceBarcodePrintHelper">  	     <form id="invoiceBarcodePrintHelperForm">  	         <div class="form">  	             <div class="header">  	                 <span>Print Barcode (Box)</span>  	             </div>  	   	             <div class="content">  	   	                 <div class="invoiceDetails">  	                     <div>  	                         <span>Invoice No</span>  	                         <span>:</span>  	                         <span id="lblInvoiceNumber"></span>  	                     </div>  	   	                     <div>  	                         <span>Products</span>  	                         <hr />  	                         <table id="tblProductsToPrint">  	                             <thead>  	                             <tr>  	                                 <th>Item</th>  	                                 <th>Count</th>  	                             </tr>  	   	                             </thead>  	                             <tbody>  	                             </tbody>  	                         </table>  	   	                     </div>  	   	   	   	                 </div><div class="formItem">     <div>Packed Date</div>     <input id="iPackedDate" required type="date"></div></div>  	   	             <div class="footer">  	                 <button id="btnPrint" type="submit">Print</button>  	                 <button id="btnCancel" type="button">Cancel</button>  	             </div>  	         </div>  	   	     </form>  	   	   	 </dialog>'
    },
    initialize: function(){
        var self = this;
        self.elements = {};
    ...
    setInvoiceDetails: function(invoice){
        var self = this;

        self.container.find(self.selectors.lblInvoiceNumber)
    ...
    initializeButtons: function () {
        ...
        self.elements.buttons.btnPrint.on('click', function(eve){
            ...
            self.doPrinting(self.config.invoice, function(err){
    ...
    doPrinting: function(invoice, doPrintingCallback){
        var self = this;
        async.mapSeries(invoice.products, function(product, next){
    ...
    show: function(invoice, callback){
        var self = this;
        self.resetForm();
        self.config.invoice = invoice;
        self.callback = callback;
        self.container.get(0).showModal();
        self.setInvoiceDetails(self.config.invoice);
        return self;
    }
...
window.invoiceBarcodePrintHelper = new InvoiceBarcodePrintHelper();
...
//
//invoiceBarcodePrintHelper.show({
//    invoiceNumber : 1002,
//    products: [
//        {
