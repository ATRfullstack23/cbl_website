<script>
    ...
    export let filter_advanced_settings;
    ...
    onMount(() => {
        ...
        console.log('filter_advanced_settings', filter_advanced_settings)

        editable_data = JSON.parse(JSON.stringify(filter_advanced_settings || {}));
        editable_items_arr = JSON.parse(JSON.stringify(Filter.ADVANCED_SETTINGS['default'].customization_config.items));
...
<div class="sql_container">
    <div bind:this={editorContainer} class="editor-container">
        ...
        <!--{#if form_view}-->
            <EditableFormItemsManager
                    ...
                    sub_module="{filter_info.sub_module}"
    ...
    <div class="button_container">
        <button class="button confirm_filter" on:click={confirm_update}>Confirm</button>
        <button class="button cancel_filter" on:click={cancel_popup}>Cancel</button>
    </div>
</div>
